...
:py:mod:`psm_utils` uses the `HUPO-PSI standardized ProForma v2 notation
...
| ``_AM(Oxidation (M))SIVM(Oxidation (M))LSM_`` 🠚 ``AM[U:Oxidation]]SIVM[U:Oxidation]]LSM``
...
.. automodule:: psm_utils.io.peptide_record
   :members:
   :inherited-members:
